...
if __name__ == '__main__':
    ...
    for ii in range(len(soulie)):
        ...
        for m in range(nubSlc):
            ...
            ###存入切片
            img_retS = np.zeros([512,512,7])
            img_retS[:,:,:-1]= img_ret
            img_retS[:,:,-1] = imgSlice
            # plt.imshow(img_retS[:,:,-1])
            # plt.show()
            np.savez('E:/code/Unet/data/dataSlice/train/data.'+str(indexPatient)+'.'+str(Slice)+'.npz',RTSt=img_retS,RTDose=ArrDose[:,:,m])
